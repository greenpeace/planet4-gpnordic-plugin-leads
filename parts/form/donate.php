    ...
    <div id="donate-container" class="donate-container">
        ...
        <?php if ($thank_you_settings['enable_donation_amount']) : ?>
            <div id="donate-input-amount" class="input-container">
                <input placeholder="Custom amount" id="ghost" class="ghost donation-options" type="number" :min="blockData.donateMinimumAmount" pattern="[0-9]*" v-model="donateAmount" @keypress="numbersOnly($event), presetDonateAmount = 0" @change="checkMinVal($event), presetDonateAmount = 0"> <span class="currency"><?php echo $form_fields_translations['donate_currency']; ?></span>
            </div>
        <?php endif; ?>
